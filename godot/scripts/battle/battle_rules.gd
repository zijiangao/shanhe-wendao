...
static func can_frost_dash(battle: Dictionary, cell: Vector2i, qi_cost: int = 6, bonus_range: int = 0) -> bool:
	...
	if absi(cell.x - start.x) + absi(cell.y - start.y) > 2 + bonus_range:
		return false
