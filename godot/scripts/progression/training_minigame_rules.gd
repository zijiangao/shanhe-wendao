...
static func _discipline_options(state: Dictionary, disciplines: Array) -> Array:
	...
	for discipline in disciplines:
		...
		if discipline == "herbalism":
			var herb_level := HERBARIUM_RULES.gather_level(state.get("herbarium", {}), HERBARIUM_RULES.lifetime_catches(state))
			gather_text = "\n采药等级 Lv.%d/%d（药材本身的图鉴等级，越高解锁越稀有的品种）" % [herb_level, HERBARIUM_RULES.MAX_GATHER_LEVEL]
		elif discipline == "mining":
			var mineral_level := MINERALOGY_RULES.gather_level(state.get("mineralogy", {}), MINERALOGY_RULES.lifetime_catches(state))
