...
func _migrate_and_validate() -> void:
	...
	if typeof(data.tutorial) != TYPE_DICTIONARY:
		data.tutorial = {"map": false, "location": false, "battle": false, "battle_tactics": false, "battle_defense": false}
	for step in ["map", "location", "battle", "battle_tactics", "battle_defense"]:
