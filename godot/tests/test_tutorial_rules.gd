...
func _initialize() -> void:
	var state := {
		"location": "qingyun",
		"tutorial": {"map": false, "location": false, "sparring": false, "battle": false, "battle_tactics": false, "battle_arts": false, "battle_defense": false},
		"battle": {}
	}
	...
	assert(RULES.step_for("battle", state) == "battle_arts", "The weapon-combo page should follow enemy tactics.")
	var arts_content: Dictionary = RULES.content("battle_arts", "unused")
	for concept in ["普通攻击", "断岳刀法", "永久削减护甲", "流云剑法", "每层追加4点"]:
		assert(concept in str(arts_content.body), "The weapon tutorial should explain %s." % concept)
	RULES.mark_seen(state, "battle_arts")
	assert(RULES.step_for("battle", state) == "battle_defense", "The defensive response page should follow weapon combinations.")
	...
	RULES.reset(state)
	assert(not bool(state.tutorial.map) and not bool(state.tutorial.sparring) and not bool(state.tutorial.battle) and not bool(state.tutorial.battle_tactics) and not bool(state.tutorial.battle_arts) and not bool(state.tutorial.battle_defense), "Reset should restore all tutorial steps.")
