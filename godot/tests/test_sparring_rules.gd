...
func _initialize() -> void:
	...
	assert(RULES.skill_gain_for_grade("S") == 2 and RULES.skill_gain_for_grade("C") == 1, "Every completed spar should train the chosen weapon, with an S-grade bonus.")
	assert(RULES.discipline_name("swordsmanship") == "剑法" and RULES.discipline_name("bladesmanship") == "刀法", "Weapon disciplines need stable player-facing names.")
