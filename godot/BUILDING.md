...
采药和挖矿等级按累计采集量保存，制作消耗材料不再降低等级或锁回稀有品种。


## 0.159.0 — full game

炼药与锻造配方显示缺少的材料、银两及持有量，解释无法制作的原因。


## 0.158.0 — full game

战斗结算防重复触发，保护奖励、战后选择及败北损失。


## 0.157.0 — full game

保护战后奖励和终章抉择，避免导航与结束本周按钮跳过未完成选择。


## 0.156.0 — full game

读档恢复终章抉择与通关结局，避免退回地图丢失流程入口。


## 0.155.0 — full game

重试战斗恢复开战前消耗品，避免失败尝试永久耗尽药品。


## 0.154.0 — full game

修复旧存档角色等级迁移，避免历史经验重复发放属性奖励。


## 0.153.0 — full game

管理页面连续切换后返回原场景，避免页面循环，并完成完整版回归验证。


## 0.152.0 — full game

恢复后的保存保留健康备份，并在写入失败时保护已有进度。


## 0.151.0 — full game

修复异常同伴经验、等级和成长属性，避免负成长及重复升级奖励。


## 0.150.0 — full game

本周行动用尽后禁用武学修炼选项，并提示休息后继续。


## 0.149.0 — full game

载入时合并重复武学，避免重复菜单和虚增战力。


## 0.148.0 — full game

读档保留主动卸下内功和身法的选择，同时兼容旧版存档。


## 0.147.0 — full game

键位设置往返保留暂停、主菜单或场景来源，避免意外跳回地图。


## 0.146.0 — full game

主菜单继续游戏识别自动存档备份，恢复中断的旅程。
...
分派任务菜单同步本周行动状态并说明结算时机


## 0.138.0 — full game

修复缺失装备栏的首次入库及异常负数量处理
...
校验战斗存档出手者并阻止倒下同伴继续行动


## 0.130.0 — full game

战斗侧栏适配可用高度避免结束回合按钮被底栏遮挡


## 0.129.0 — full game

战斗侧栏支持滚动及键盘焦点跟随以容纳全部武学
...
战斗操作提示显示实际轻功距离和同伴招式名称


## 0.126.0 — full game

新行动回合重置操作选择避免角色专属武学残留


## 0.125.0 — full game

修正普攻破绽实际层数反馈及同伴错误继承刀法加成
...
As of 2026-08-11, 战斗改为身法驱动的逐单位行动条 (0.121.0), 用户要求"战斗改成版回合制，根据身法决定出招速度，最好有个进度条，横板进度条，在战斗屏幕上方"。这是迄今为止最大的一次核心玩法重构：原本是"每回合2点共享行动点，自由分配给沈羽/同伴，然后点结束回合让所有敌人一次性批量行动完"，完全没有逐单位的出手顺序，`agility`（身法）属性在战斗里也完全无作用（移动范围加成实际来自独立的轻功技能树）。经两轮确认锁定范围：彻底改成逐个单位行动的行动条（不是轻量级的"只加排序参考线"），身法直接用 `agility` 属性驱动（不与轻功合并），开局不特殊处理、完全按身法排序（身法快的敌人可能比沈羽先手），敌人仍然"移动或攻击二选一"不新增行动点预算。`battle_engine.gd` 新增 `ATB_THRESHOLD=100`、`_living_units()`/`_tick_until_ready()`（行动条tick循环，`advance_turn()`与`preview_queue()`共用）、`advance_turn(battle)`（推进真实行动条，沈羽/同伴轮到自己补满`action_points`，轮回到沈羽时`battle.turn`自增）、`preview_queue(battle,count)`（纯函数，UI预览专用）；原来批量处理全体敌人的`enemy_turn()`收窄成只处理一个敌人的`resolve_enemy_turn(battle,enemy_index,hero_hp,rng,hero_armor)`，逻辑本体不变，`main.gd`新增`_advance_battle_queue()`循环驱动"敌人依次行动直到轮到沈羽/同伴"。`battle.ap`全局改名`action_points`（旧名暗示已不存在的共享池语义）。`battle_rules.gd`的`is_heavy_turn`/`is_aimed_shot_turn`/`is_boss_sweep_turn`从"读`battle.turn`取模"改成"读敌人自己的`actions_taken`取模"，新增`enemy_speed(enemy)`（boss二阶段+2，仿照`enemy_move_steps()`已有的"二阶段更快"写法）；弓手压制从"下回合行动点降到1"改成直接扣被命中一方的行动条（ATB式减速）。同伴身法复用`CompanionRules.apply_gear_and_move()`里此前从未被读取的目录`agility`字段，跟人物界面"身法"栏显示的数值完全一致。战斗屏幕上方新增横板行动条（`tactical_battle_view.gd`的`_build_turn_order_bar()`，复用战棋网格已验证的Button+图标写法），删除了从来没真正切换过文字的旧"我方回合"横幅，棋盘/侧边栏整体下移44px腾位置。`SAVE_VERSION`10→11，老存档的进行中战斗因为缺少`action_points`字段会被`_valid_battle()`自然拒绝、重置为空（不写专门迁移代码，版本更新时若玩家正在战斗中会丢失那一场进度，可接受）。测试改动面极大：`test_battle_engine.gd`/`test_battle_rules.gd`几乎全文件重写（fixture补身法/gauge字段，`battle.turn`取模的用例改用`actions_taken`直接播种，`_test_survival_objective`/`_test_enemy_movement_and_turn_reset`等概念性重写，新增`_test_turn_order_queue()`直接验证出手顺序/同gauge优先级/精确扣减/预览纯函数性质）。

As of 2026-08-11, 背包页面改版为图标网格 + 详情面板 (0.120.0), 用户反馈"物品页面有点怪，看看怎么能改一点"。代码走查确认根因：`_backpack_row()` 的文字 Label 从未设置过 `autowrap_mode`，而 0.119.0 又给每件装备行拼上了"拥有 N 件 · 已被同伴装备 N 件"、特殊材料行更是拼上完整物品描述，固定52px行高下这些新文字很容易被截断/挤变形。经确认后，改动方向不是简单修换行，而是把整个背包从"竖排整行列表"改成图标网格（用户原话"可以改成网格式的吗"），同时统一"材料与药品"分区的显隐规则（此前无论数量是否为0都固定显示四行，是全屏唯一的例外，这次改成和其余分区一样"没有就隐藏"）。面板从 `Vector2(910,520)` 放大到 `Vector2(970,520)`（对齐藏经阁已有尺寸），改成 `HBoxContainer` 左右分栏：左侧沿用原有7个分区（已装备/其余兵刃/其余护具/已习招式/已修炼内功轻功/其余已习内功轻功/材料与药品/特殊材料），每个分区标题后跟一个 `GridContainer`（5列）的图标格子；右侧新增 `detail_panel` 展示当前选中物品的完整信息+装备按钮。新增 `_backpack_describe_item(category, id, equipped)` 把"查目录数据"和"画格子/画详情"彻底拆开——格子（`_backpack_grid_cell()`）与详情面板（`_refresh_backpack_detail_panel()`）都只从这一个函数取数据，不再各自重复查询逻辑；这也是原来四个行构建函数（`_backpack_equipment_row`/`_backpack_wuxue_row`/`_backpack_goods_row`/`_backpack_specimen_row`）与通用 `_backpack_row()` 被删除、逻辑合并进来的地方。格子构建直接照抄战棋网格（`main.gd` 战斗格子构建函数）已验证的写法：`Button` 原生带 `icon`/`text`，按状态换 `stylebox`，`pressed` 绑定选中——不是直接装备，只是选中并刷新详情面板，装备始终是详情面板里那个明确的按钮，避免误触网格换装。新增 `backpack_selected` 状态变量（只在从其他页面切入背包时于 `_switch_screen()` 里重置，背包内部装备操作触发的重建不会打断它），以及 `UI_THEME.selectable_box(color, selected)`（`ui_theme.gd` 新增，`box()` 的可选中变体，选中时描边变金色）。测试更新 `tests/test_backpack_view.gd`，全部从搜索 Label 文字改成用格子的稳定 `name`（`backpack_cell_<category>_<id>`）定位，新增"材料与药品全空时整个分区隐藏"的回归测试。

As of 2026-08-11, 兵器/护具改为数量制，同一件的多份可分给不同人穿 (0.119.0), 用户要求"武器防具需要有数量，在背包里显示，特殊材料也需要有数量。如果只有一件武器，那被装备了，其他人就无法装备"。经确认两点：多份拥有的获得方式是西市/锻造坊可以重复购买/打造同一件（此前 buy_weapon()/CraftingRules.can_craft() 都明确拒绝"已拥有"的重复购置/打造）；且0.113.0"同一件装备可以多人同时装备，不排他"的设计前提不再成立，改为按数量排他。`owned_weapons`/`owned_armors` 从"不重复id数组"改为"id → 拥有数量"字典。新增文件 `scripts/progression/equipment_rules.gd` 作为数量与"谁在穿"归属的单一权威（`owned_count()`/`add_owned()`/`wearers()`/`claimed_count()`/`is_available_for()`/`reconcile_wearers()`），刻意拆成独立文件是因为 `shop_rules.gd` 已经 preload 了 `crafting_rules.gd`，两者互相 preload 会形成循环依赖，而这个文件零依赖，三者（`shop_rules.gd`/`crafting_rules.gd`/`companion_rules.gd`）都能安全 preload 它。`buy_weapon()`/`buy_armor()`/`CraftingRules.apply()` 的打造分支现在都允许重复购买/打造，各自增加一份数量；`equip_weapon()`/`equip_companion_weapon()`（及护具对应函数）现在都要求"还有一份没被别人占用"才能换装，卖出装备时 `reconcile_wearers()` 会在数量降到低于当前穿着人数时自动卸下多余的人（沈羽优先，其余按同伴加入顺序）。人物界面同伴换装列表、西市购买列表、背包装备行都新增拥有份数与"已被同伴装备X件/均已被占用"展示。背包新增"特殊材料（药谱/矿谱）"分区，把已发现的药谱/矿谱稀有品种连同数量一并展示——此前这些数量只在采集结果卡与炼药坊/锻造坊配方说明里露面，背包本身从未展示过。测试覆盖新文件 `tests/test_equipment_rules.gd`（数量增减、排他校验、卖出后的强制卸下逻辑）与更新后的 `tests/test_shop_rules.gd`/`tests/test_companion_rules.gd`/`tests/test_crafting_rules.gd`/`tests/test_game_state.gd`（含老存档数组格式→新字典格式的迁移覆盖）。

As of 2026-08-10, 炼药坊/锻造坊新增独立的1-10级工坊等级 (0.118.0), 用户要求"锻造坊连药坊也应该有类似升级机制"（跟进 0.117.0 采药/挖矿的图鉴等级）。经确认两点：经验来源用新增的独立累计打造/炼制次数计数（而非直接复用采集图鉴等级——锻造坊现有4件兵刃/护具都只能各打造一次，不能像采药挖矿那样无限重复累积，需要一套自己的计数）；解锁内容除了给已有配方设门槛外，还要同时新增全新的更高阶丹药/兵器。`crafting_rules.gd` 新增 `craft_level()`/`RECIPE_LEVEL_REQUIREMENT`/`MAX_CRAFT_LEVEL=10`，计数存于新增的 `alchemy_crafts`/`forge_crafts` 两个整数字段（`GameState.new_game()` 里默认0，读取处全部走 `state.get(key,0)` 保证旧存档兼容）。`CRAFTS_PER_LEVEL` 刻意设为1（而非采药挖矿的3）：锻造坊的顶尖配方需要5级门槛，若用更大步长，光靠打造现有4件兵刃护具（forge_crafts封顶4）永远够不到5级，会造成"新配方永远解锁不了"的死锁——1点/级让"把现有全部配方打造一遍"正好凑满5级，门槛精确对齐。新增配方顺带给 0.117.0 引入却一直没人用的10级图鉴稀有品种（七星莲/星陨髓）派上真正用场：`vitality_pill`（回元丹，四维属性各+1，需七星莲）、`star_marrow_blade`（星陨寒锋，攻击+5）、`star_marrow_armor`（星陨玄甲，防御+4，均需星陨髓）。已有的5款丹药/4款兵刃护具门槛保持1级不受影响（它们已被0.117.0的采集图鉴等级间接把关过）。炼药坊/锻造坊选择列表与材料说明行都新增等级展示与"尚需X级"提示。测试覆盖 `tests/test_crafting_rules.gd`（等级推算、硬门槛拒绝未解锁顶尖配方、达标后解锁）。

As of 2026-08-10, 采药/挖矿新增独立的1-10级"图鉴等级" (0.117.0), 用户要求"采药挖矿增加升级系统，最高十级，等级越高解锁高级矿物药材"。经确认，这个等级与 `TrainingMinigameRules` 里采药人/挖矿人本身0-100级的专精熟练度（影响小游戏难度/产量）是两回事——这里指的是药材/矿物本身的收藏图鉴等级；且"解锁"是硬门槛，未到等级即使单局小游戏打出S级也绝对采不到高阶品种，不是概率加成。`herbarium_rules.gd`/`mineralogy_rules.gd` 新增 `gather_level()`/`LEVEL_UNLOCK`/`MAX_GATHER_LEVEL=10`/`CATCHES_PER_LEVEL=3`：等级直接从收藏字典（`herbarium`/`mineralogy`）的采集总数推算（`1 + 总采集数/3`，封顶10），不需要新增存档字段，天然兼容旧存档。`record()` 原本按小游戏成绩(C/B/A/S)决定候选品级池 (`GRADE_POOLS`)，现在额外用当前等级（叠加前的等级，即这次采集本身不能用它刚刚跨过的新等级来挑选品级）对该池做 `LEVEL_UNLOCK` 过滤——四个品级门槛分别是1/4/7/10级，与C/B/A/S四档粗略对应但相互独立。演武场/后山的训练小游戏结果卡新增等级/升级提示文案，`gathering_options()` 的说明文字也新增当前等级展示。测试覆盖 `tests/test_herbarium_rules.gd`/`tests/test_mineralogy_rules.gd`（等级推算公式、硬门槛拒绝未解锁品级、跨级瞬间的边界行为）。

As of 2026-08-08, 侠客新增独立升级系统 (0.116.0), 直接取代 0.115.0"分派任务·修炼"里那个封顶10点的 `attack_bonus` 单一数值——用户明确要求"侠客也要能升级"，经确认后设计为：经验来源两处都算（分派任务的"修炼"任务，以及跟随沈羽参战获胜），成长内容沿用沈羽自己的四维属性成长（每级臂力/身法/悟性/根骨各+1），人物界面同样显示等级数字。共用 `GrowthRules.character_level()`/`LEVEL_XP_STEP=25` 的公式与节奏，但数据完全独立存放在 `companion_growth[id]`（不影响沈羽自己的 `xp`/`character_level`）——`WeeklyTaskRules.grant_companion_xp()` 是唯一入口，"修炼"任务复用其原有的 `COMPANION_TRAIN_XP=12`（与沈羽 `apply_training()` 发放的12点修为一致），`GameState.finish_battle()` 新增战斗胜利分支，对 `qingyun_spar` 的当前随行弟子、以及 `huashan_trial`/`wuku_finale` 里的林清霜授予与沈羽本人相同的战斗修为。攻击成长读 `strength` 差值，气血成长读 `constitution` 差值×3（跟沈羽自己 `grant_xp()` 的+3/级同构），二者都叠进 `CompanionRules.apply_gear_and_move()` 算出的 `ally.attack`/`ally.hp`/`ally.max_hp`。人物界面左侧名单按钮与同伴信息卡都新增"Lv.N"等级展示，基础属性格子改为读 `CompanionRules.companion_display_attribute()`（目录基础值+累积成长）而非纯目录固定值。测试覆盖 `tests/test_weekly_task_rules.gd`（无封顶的持续升级、等级边界处属性变化）、`tests/test_game_state.gd`（切磋胜利授予随行弟子经验）。

As of 2026-08-04, 沈羽与同伴新增"分派任务" (0.115.0), 一个与演武场/后山小游戏并列的新选项——用户明确要求保留旧的方向连招/蓄势计时小游戏，只是新增一条不用玩小游戏、直接自动结算收益的路径，绝非替换。任务分三种：赚钱/修炼/采集，`scripts/progression/weekly_task_rules.gd` 是唯一权威。沈羽这份是一次性的：`GameState.assign_hero_task()` 立即花掉本周行动（跟其余每周一次的行动互斥），但收益要等 `end_week()` 才真正结算，结算完 `weekly_task_hero` 清空，下周需要重新选。同伴完全没有行动点概念，`assign_companion_task()` 不花任何东西，设定后持续生效，直到玩家改派或取消（设成空字符串），每次 `end_week()` 都会重新结算一次——这是同伴第一次真正拥有持续成长手段：`companion_growth[id].attack_bonus` 靠"修炼"任务累积，封顶10点，叠进 `CompanionRules.apply_gear_and_move()` 算出的 `ally.attack`。"修炼"任务复用了此前完全没被任何UI调用过的死代码 `GrowthRules.apply_training()`（`GameState.train()` 本身也从未被接线，这次直接绕开它调用底层函数，避免重复花费行动点）。人物界面沈羽面板新增"本 周 任 务"板块与"分派任务"按钮，同伴信息卡在装备按钮之后新增同名板块。测试覆盖新文件 `tests/test_weekly_task_rules.gd`（三种任务的结算公式、修炼封顶、选项列表的已分派/持续中标记）与 `tests/test_game_state.gd`（沈羽一次性 vs 同伴持续性的完整 `end_week()` 集成流程、未入门弟子拒绝分派）。

As of 2026-08-04, 同伴现在也能更换内功与轻功 (0.114.0), 同一天对 0.113.0 装备/武学的直接延伸。跟兵器/护具/招式同样的思路，同伴从沈羽已学的 `learned_internal`/`learned_lightness` 里借一门，且沿用沈羽自己已经修炼到的等级（`WuxueRules.internal_level()`/`lightness_level()`），不是同伴另起炉灶重练一遍。新增 `CompanionRules.companion_internal_damage_bonus()`/`companion_lightness_move_bonus()`，结构照抄 `WuxueRules.internal_damage_bonus()`/`lightness_move_bonus()`的形状但读取同伴选定的技艺id而非沈羽自己的 `equipped_internal`/`equipped_lightness`。内功加成叠进 `apply_gear_and_move()`算出的 `ally.attack`；轻功加成写进新的 `ally.lightness_bonus`字段，用来扩展突进技的最大射程——`battle_rules.gd`的 `can_frost_dash()`新增 `bonus_range`参数（追加在末尾，不是插入中间，沿用本项目一贯的这个惯例），`battle_engine.gd`新增 `ally_dash_bonus_range()`读取该字段并传给 `_frost_dash()`内外两处 `can_frost_dash()`调用点，以及 `main.gd`战场网格高亮那一处调用点。人物界面同伴信息卡的"武学"板块新增"更换内功"/"更换轻功"两个按钮（原先只有"更换武学"一个），跟已有的换装按钮同一行排列。测试覆盖 `test_companion_rules.gd`（内功/轻功习得校验、等级继承计算）与 `test_battle_engine.gd`（轻功加成真正扩大突进射程的验证，比照沈羽自己轻功加成扩大移动范围的既有测试写法）。

As of 2026-08-04, 同伴（林清霜与客栈弟子）现在可以真正更换装备与武学 (0.113.0), extending 0.111.0's display-only roster panel into a functional one. 装备共用沈羽自己的 `owned_weapons`/`owned_armors` 池 (`CompanionRules.equip_companion_weapon()`/`equip_companion_armor()`) -- 同一件兵器/护具可以被多人同时装备（不是排他所有权），换装不需要新的购置渠道。武学则从沈羽的 `learned_moves` 里选一招 (`equip_companion_move()`) 替代同伴默认的"霜华刺"突进技，真正改变战斗表现：`battle_engine.gd` 新增 `ally_dash_title()`/`ally_dash_qi_cost()` 从 `battle.ally.move_id`（若设置）读取 `WuxueRules.MOVES` 里对应招式的名号/真气消耗/`level_damage_bonus`，未选择时行为与之前完全一致（默认霜华刺，6真气，无额外加成）。`CompanionRules.apply_gear_and_move()` 是两处基础 ally 数值的共同叠加入口——林清霜（`game_state.gd`'s `start_huashan_trial_battle()`/`start_final_battle()`，两处硬编码基础数值）与客栈弟子（`active_disciple_ally()`）都调用它，保证换装/换武学效果在两条路径上一致。护具加成同时接入了 `enemy_turn()` 里此前完全没有护甲减免的同伴受击分支（之前只有护卫值吸收，现在护甲先减免、护卫再吸收，跟沈羽自己的受击顺序一致）。人物界面的同伴信息卡新增"更换武学"/"更换兵器"/"更换护具"三个按钮，打开与秘籍阁/西市同款的抉择列表；`main.gd` 新增 `companion_gear_target` 记录当前在为哪位同伴选装备。测试覆盖 `test_companion_rules.gd`（装备所有权校验、武学习得校验、共享装备池不互斥）、`test_battle_engine.gd`（自定义突进技的真气消耗/伤害加成/护甲减免）、`test_game_state.gd`（林清霜在华山试炼里真正带上装备加成）。

As of 2026-08-04, fixed a real text-clipping bug on the character screen's portrait identity label (0.112.0), caught immediately after 0.111.0 shipped by the user pointing at their own screenshot ("青云门·入门弟子" rendering as "青云门·入门弟" with the last character cut off). Root cause: the `identity` Label under the portrait had a raw `position = Vector2(22, 18)` with no explicit size/wrap, so its natural (unwrapped) text extent could exceed `portrait_frame`'s `clip_contents = true` boundary at some window sizes -- reproduced headlessly at 1264×706 (a window a shade smaller than the usual 1280×720 test size, closer to what a real windowed desktop app renders at). Switched `identity` to `set_anchors_and_offsets_preset(PRESET_FULL_RECT)` with explicit left/top/right offsets and `autowrap_mode = TextServer.AUTOWRAP_WORD_SMART`, so it always wraps within its shaded strip instead of silently overflowing past the clip boundary; bumped `identity_shade`'s height from 112 to 128px to comfortably fit the resulting 3-line wrap for longer identity strings. Verified by re-rendering the same 1264×706 headless screenshot that reproduced the bug, for both 沈羽 and a selected companion (0.111.0's new roster panel shares this same portrait/identity code).

As of 2026-08-04, 人物界面 (character screen) became a left/right roster browser (0.111.0), mirroring 藏经阁's pattern (`_show_library()`): a narrow left column lists 沈羽 plus every joined companion (林清霜 if recruited via the 华山 quest beat, and *every* 客栈-recruited disciple, not just the currently active one), and clicking a name swaps the right side's portrait + info panel to that character. This directly answers the user's ask ("每一个侠客都应该有一个相同面板，包括基础属性/武学/装备") after they circled the existing hero-only panel and asked for parity. New `character_roster_selection` member var (`main.gd`) tracks the current selection, resetting to `"hero"` if the previously-selected companion is no longer valid (e.g. after a save reload).

Companions previously had only combat-formula fields (`hp`/`max_qi`/`attack`/`guard`, enough to build a `battle.ally` dict) -- no 臂力/身法/悟性/根骨, no 武学, no 装备, since nothing ever displayed those for them. `CompanionRules.DISCIPLES` gained `strength`/`agility`/`insight`/`constitution` plus `skills_text`/`gear_text` flavor strings, and a new `STORY_COMPANIONS` dict holds the same shape for 林清霜 (previously she had **no** persistent catalog entry at all -- her stats are hardcoded inline, with two different sets of numbers, directly inside `start_huashan_trial_battle()`/`start_final_battle()` in `game_state.gd`). This is a deliberately scoped interpretation of "同一套系统": these new fields are **display-only** flavor for the roster browser's panel -- they don't feed a new companion-specific training/shopping economy (companions don't level up or buy gear of their own; only the hero does). `main.gd`'s new `_show_companion_info()` renders the exact same panel *shape* as 沈羽's own (综合数值 summary → 基础属性 stat-card grid → 武学 box → 装备 box), just sourced from this fixed catalog instead of live growing save data. The roster's portrait art reuses `portrait-lin-qingshuang.png` as a shared placeholder for every companion (no new art was commissioned) -- a known, accepted cosmetic mismatch for non-林清霜 companions, flagged rather than silently accepted. `tests/test_companion_rules.gd` covers `roster()`/`companion_entry()`/`is_valid_companion()`; `tests/test_character_view.gd` extended to cover the solo-hero roster (one button) and the full roster (clicking 周慕白 switches the panel and shows his stats).

As of 2026-08-04, 任务界面 (quest screen) now shows the same 同行侠客 panel as 人物界面 (0.110.0), so a 客栈-recruited disciple shows up there too instead of only being mentioned inline in the 华山 quest stage's own text (which previously said "队伍：沈羽、林清霜" and never mentioned recruited disciples at all). Extracted `_party_names()`/`_party_panel_text()` helpers (`main.gd`) so both screens build the exact same "同行侠客：X、Y\n门派关系：..." text and render it in the identical boxed panel style (`_box(Color("#223a30"))`, matching 人物界面's 武学/材料 panels) -- previously the character screen's party line was a bare colored Label with no box at all, and the quest screen had no party display outside the one hardcoded 华山-stage sentence. New `tests/test_quest_view.gd` covers both the "暂无" solo state and a recruited disciple actually appearing.

As of 2026-08-04, added a 客栈 (tavern) hotspot inside 青云门 where the hero can recruit disciples (徒弟) with silver, who then accompany him into repeatable 切磋 (`qingyun_spar`) sparring battles (0.109.0). New `scripts/progression/companion_rules.gd` (`CompanionRules`) holds a small catalog (`DISCIPLES`, currently 周慕白/柳如烟) plus `recruit()`/`options_inn()`/`active_disciple_ally()` -- structurally similar to `ShopRules`, but this is deliberately **not** a generalization of the existing story companion 林清霜: `data.companions` still holds `"lin_qingshuang"` exactly as before, and `start_huashan_trial_battle()`/`start_final_battle()` (the two battles she fights in) are untouched. Recruited disciples are a separate, additive roster tracked by a new `data.active_disciple` field (whichever was most recently recruited automatically becomes the one who fights alongside the hero) and only ever plugged into `start_qingyun_spar_battle()`'s `battle.ally`, which previously had no ally at all. This scoping choice deliberately avoids touching 林清霜's quest-critical hardcoded battle integration.

Because a disciple other than 林清霜 could now legitimately be `battle.ally`, every hardcoded `"林清霜"` string literal in `battle_engine.gd` (five move-rejection messages, `_frost_dash`/`_frost_guard`'s flavor text, `_active_name()`, `_turn_result()`'s defeat message) and `tactical_battle_view.gd` (`active_name` derivation and the `active_name == "林清霜"` comparisons gating the ally action bar) were generalized to read `battle.ally.name` (falling back to "同伴") instead -- a mechanical text-generalization, not a structural change, since the ally mechanic itself (`active_unit == "ally"`, clicking the ally's grid token to switch control) was already data-driven. `main.gd`'s "当前由林清霜行动" grid-click message and the character sheet's "同行侠客" line were updated the same way; the ally's grid token still reuses the existing `portrait-lin-qingshuang.png` art as a placeholder for recruited disciples (no new art was commissioned for this feature). New hotspot recruiting is a pure silver transaction like 秘籍阁 -- it does **not** spend the week's one action, unlike training/gathering/crafting/battles. `tests/test_companion_rules.gd` (new), `tests/test_qingyun_tavern_view.gd` (new), and extensions to `test_game_state.gd` cover the recruit flow, the migration repair for a stale `active_disciple`, and confirm a solo (no-disciple) spar still has no `battle.ally` at all, matching pre-0.109.0 behavior exactly.

As of 2026-08-02, trimmed three redundant/stale sections off the character screen (0.108.0): 江湖技艺's "修炼战绩"/"实战切磋" lines (raw score/streak dumps that duplicated what the 演武场 training menu and quest journal already surface more usefully) and the entire "行囊" section (equipped weapon/armor/materials/herbarium/mineralogy/items text) were removed outright, since 行囊 duplicated the dedicated 背包 screen one-for-one. `party`(同行侠客/门派关系)/`mastery`(武学熟练度) Labels, which sat right after 行囊 in the same `info` VBoxContainer but aren't inventory content, were left in place untouched. `test_character_view.gd` was rewritten to assert the removed content's absence (`"行 囊"`/`"已装备："`/`"剧情物品"`/`"修炼战绩"`/`"实战切磋"` must never appear) rather than testing weapon-equip display, since that coverage already exists in `test_backpack_view.gd` for the screen that now owns it exclusively.

As of 2026-08-02, fixed a real overflow bug on the character screen (0.107.0) surfaced right after 0.106.0 shipped: `_show_character()`'s `specialties` Label (`main.gd`) had no `autowrap_mode`, so once its text grew to cover all six disciplines (拳掌/枪棍 added in 0.106.0) its single unwrapped line forced the whole info panel wider than the 1280px viewport, clipping the fourth stat card and the tail of the training-records line off the right edge. Added `autowrap_mode = TextServer.AUTOWRAP_WORD_SMART` to both `specialties` and the `summary` Label above it (same latent risk, not yet triggered), and split the specialties text across an extra line so 拳掌/枪棍 (which the 0.106.0 change had also forgotten to add to this particular display string, unlike the training-menu list) now show alongside 剑法/刀法/采药/挖矿. Diagnosed by rendering an actual headless screenshot of the character screen (`main_scene.get_viewport().get_texture().get_image().save_png()`, same technique `test_character_view.gd` already uses) rather than guessing from code alone -- confirmed the fix visually before shipping.

As of 2026-08-02, 演武场新增拳掌/枪棍两项训练，四项战斗专精（拳掌/剑法/刀法/枪棍）都真正影响对应武学威力 (0.106.0). `TrainingMinigameRules.DISCIPLINES` gained `"fistsmanship"`（拳掌 · 寸劲淬体，复用刀法的蓄势计时机制）和 `"staffsmanship"`（枪棍 · 连枪如龙，复用剑法的方向连招机制）——两者的 `challenge()`/`score_challenge()`/`score_challenge_variant()`/`timing_feedback_variant()`/`timing_feedback()` 都直接复用各自母机制的分支结构，只是各带一套自己的文案和 `ideal_ms`。`options()`（演武场列表）从两项扩展为四项；`WEEKLY_FOCUS_ORDER` 从4项轮换扩展为6项。新增两个大成(100级)顶点效果：`fist_qi_discount(level)`（拳掌大成后裂石拳真气消耗5→3）和 `spear_full_pierce(level)`（枪棍大成后裂甲枪从护甲减半升级为完全无视护甲）。

由于枪棍此前没有任何招式（`WuxueRules.MOVES` 里从没出现过 `category: "staff"`），本次新增了首个枪棍招式**裂甲枪** (`armor_splitting_spear`, `battle_engine.gd`'s `_armor_splitting_spear()`/`spear_damage_range()`) —— 效果是"当次攻击护甲减半"(`armor / 2`)，与流云剑法/裂石拳的"无视全部护甲"、断岳刀法的"永久移除护甲"三足鼎立，枪棍大成后再升级为完全无视护甲。`stone_fist_damage_range()` 也补上了此前完全缺失的专精加成项 `fistsmanship / 2`，与剑法/刀法的写法完全对称。战斗动作栏（`tactical_battle_view.gd`）、战场网格高亮（`main.gd`'s `_battle_cell_data()`）、`hero_action_help()` 均按裂石拳/暗夜三刀的既有模式追加了裂甲枪分支——招式取消装备槽 (0.104.0) 之后，秘籍阁/藏经阁/背包页面全部通用读取 `WuxueRules.MOVES`/`learned_moves`，裂甲枪加入目录后这些页面无需额外改动即自动展示。

演武场"以剑应战/以刀应战"的切磋(qingyun_spar)功能本次**不扩展**支持拳掌/枪棍，仍只有剑/刀两个选项——这是范围外的独立功能，用户只要求演武场训练本身新增两项。

`GameState.new_game()`/`power()`/`_migrate_and_validate()` 里遍历四项专精的固定列表都扩展为六项（含 herbalism/mining）；`fistsmanship`/`staffsmanship` 复用与另外两项专精完全相同的等级制/经验值制机制 (0.105.0)，无需任何新代码路径。测试更新覆盖 `training_minigame_rules.gd`/`battle_engine.gd`/`game_state.gd`/`wuxue_rules.gd`/`manuals_view.gd`（购买列表招式数量从8项变成9项）等文件。

As of 2026-08-02, 剑法/刀法/采药/挖矿 became a proper leveled system capped at 100 (0.105.0), replacing the old uncapped raw stat that grew by a flat +1~+3 per training session forever. `TrainingMinigameRules` gained `MAX_SPECIALTY_LEVEL := 100`, `specialty_xp_needed(level)` (a gentle curve, `2 + level / 25`, roughly ~350 total xp to reach level 100), and `train_specialty(state, discipline, xp_gain)` -- structurally a copy of `WuxueRules._train()`'s accumulate-xp-then-level-up pattern, but with its own 100-level curve and its own xp dict (`state.specialty_xp`, keyed by discipline, parallel to `WuxueRules`' `wuxue_xp`). The four disciplines still store their **level** directly in the same field names as before (`data.swordsmanship` etc.) -- only the semantics changed, from "unbounded raw stat" to "current level, 0-100" -- so every existing `/2`-style damage/gathering formula in `battle_engine.gd` kept working unmodified; capping the input naturally caps the output (max +50 instead of unlimited), which is exactly what "封顶" should do without a formula rewrite. 熟手/精通/大成 mastery thresholds were rescaled 10x from 3/6/10 to 30/60/100 (`SPECIALTY_RANKS`), and every threshold check gated on the old `>= 10` (`cloud_qi_cost`/`attack_exposure_gain`/`medicine_mastery_bonus`/`craft_ore_discount`, plus one hardcoded check in `battle_engine.gd`'s `blade_armor_break()` that wasn't already delegating to a `TrainingMinigameRules` helper) moved to `>= 100`. Both growth entry points -- `GameState.complete_training()` (演武场/后山 minigames) and the 切磋 sparring-victory path -- now route through `train_specialty()` instead of directly incrementing the raw field. `_migrate_and_validate()` gained an upper clamp (old saves that had grown past 100 under the removed cap get clamped down, no refund) alongside the existing lower clamp, plus repair of a malformed `specialty_xp` field. `_discipline_options()`'s 演武场/后山 menu text now shows an xp progress fraction (`"X/Y 经验"`) alongside the existing rank/level/perk text, mirroring `WuxueRules`' training-row style. Tests across `test_training_minigame_rules.gd`, `test_game_state.gd`, `test_battle_engine.gd`, `test_crafting_rules.gd`, `test_crafting_view.gd`, `test_qingyun_spar.gd`, and `test_training_minigame_view.gd` were updated for the rescaled thresholds and xp-based leveling -- notably, a single S-grade training/spar victory no longer guarantees crossing a rank boundary the way a flat +1~+3 direct increment used to, so mastery-threshold tests now pre-seed `specialty_xp` to land exactly on the boundary rather than relying on one lucky roll.

As of 2026-08-02, 招式类武学 no longer need to be "equipped" -- every learned move is usable in battle immediately (0.104.0), removing `WuxueRules.MAX_EQUIPPED_MOVES`'s two-slot cap entirely. Previously a player who learned a third move had to pick which two to equip via 背包/秘籍阁 before it would work in the tactical action bar; now `learn_move()` just appends to `learned_moves` and every combat-move gate (`BattleEngine._cloud_skill/_blade_skill/_stone_splitting_fist/_night_triple_blade`, `TacticalBattleView`'s action-bar buttons, `hero_action_help()`'s skill-card text, `GameState.power()`'s wuxue contribution) reads `learned_moves` directly instead of a separate `equipped_moves` field. `WuxueRules.equip_move()`/`unequip_move()` were deleted outright (内功/轻功's `equip_internal/lightness`/`unequip_internal/lightness` are untouched -- those stay single-slot, since replacing one internal art/lightness skill with another is a real choice, unlike a move capacity limit that just added friction). `options_manuals()`'s move rows show a read-only "已习得" state instead of "装备"/"卸下"/"槽位已满"; `options_library_category()`'s move rows no longer carry an "（已装备）" suffix (内功/轻功 rows still do). `main.gd`'s backpack screen merged "已修炼武学"/"其余已习武学" into a single "已习招式" section with no equip buttons, and its 秘籍阁 purchase routing dropped the `equip_move_`/`unequip_move_` prefixes. `GameState._migrate_and_validate()` no longer filters/clamps an `equipped_moves` field, and `new_game()` no longer initializes one -- old saves with a stale `equipped_moves` key simply have it ignored, since nothing reads it anymore. `test_wuxue_rules.gd`, `test_battle_engine.gd`, `test_game_state.gd`, `test_manuals_view.gd`, `test_backpack_view.gd`, and `test_library_view.gd` were all updated for the learned-only gate, including a `test_battle_engine.gd` case that specifically learns four moves at once (impossible under the old two-slot cap) and confirms one still works, as the real behavioral proof the limit is gone rather than just a field rename.

As of 2026-08-01, 藏经阁 became a dedicated left/right category browser (0.103.0) instead of one long grouped list -- left column has 拳掌/剑法/刀法/枪棍/内功/轻功 buttons, clicking one shows that category's learned wuxue on the right. This replaced the old `screen == "choice"`/`choice_event == "library"` flow with a fully custom `screen == "library"` view (`main.gd`'s `_show_library()`), matching the pattern already used by `_show_achievements()`/`_show_quests()`/`_show_character()`, since a left/right split with independently-clickable category buttons doesn't fit the generic `ChoiceView`'s single vertical button-list abstraction. `WuxueRules.options_library()`/`_library_header()` (the 0.102.0 flat grouped-list function) were removed and replaced with `options_library_category(state, category_id)`, which returns just one category's rows (a disabled placeholder row when nothing is learned in it yet) for `"fist"/"sword"/"blade"/"staff"` (filtered via `WuxueRules.MOVES`' `category` field) or `"internal"/"lightness"` (handled separately, since those aren't move categories at all). The new `"library"` screen was deliberately left unwired in `NavigationRules` (not added to `OVERLAY_SCREENS`/`PAUSABLE_SCREENS`/`back_action()`'s targets), matching the pre-existing precedent that `"backpack"` has the same gap and relies solely on its own explicit UI "返回" button. `test_wuxue_rules.gd` and `test_library_view.gd` were rewritten for the new per-category API and left/right UI respectively.

As of 2026-08-01, 藏经阁 groups learned moves under 拳掌/剑法/刀法/枪棍 category headers (0.102.0) -- purely a display grouping, `WuxueRules.MOVES` entries gained a `category` field (`fist`/`sword`/`blade`/`staff`) with no gameplay effect. Categories with nothing learned in them are skipped entirely rather than shown with an empty-state placeholder, per explicit choice -- no 枪棍 move exists in the game yet, so that heading simply never appears until one is added. `options_library()` iterates `WuxueRules.CATEGORIES` in a fixed display order (拳掌→剑法→刀法→枪棍), grouping each category's learned moves under a disabled header row before falling through to the existing 内功/轻功 rows and the leave row. `test_wuxue_rules.gd` updated for the new row counts and extended to assert the 枪棍 header never appears with the current catalog.

As of 2026-08-01, 调息 was removed outright rather than relocated (0.101.0) -- `GameState.end_week()` now restores hp/qi to full automatically every time the week is ended, so the standalone rest action is no longer needed. `GameState.rest()` was deleted entirely (its old body's heal logic moved into `end_week()`); `main.gd`'s `rest_button`/`_rest_requested()` (added just one release earlier, in 0.100.0) were removed along with their `_update_status()` sync block. `end_week_button`'s tooltip was reworded to mention the automatic recovery. `test_game_state.gd`'s `rest()`-specific assertions were replaced with a direct check that `end_week()` restores hp/qi from a damaged/depleted state.

As of 2026-08-01, 天下舆图's side panel was removed entirely (0.100.0) -- the last remaining pieces, the 进入.../调息/江湖纪事 buttons, are gone from the map screen. Entering the current location now happens by clicking its own map marker (the marker button already emits `destination_requested` with the hero's own current location id, which `_map_destination_requested()` already special-cased as "enter" rather than "travel" since long before this change -- no new code path needed). 调息 moved to a new persistent header button (`rest_button`, next to `end_week_button`, same disabled/tooltip-sync pattern in `_update_status()`, same `NAVIGATION_RULES.blocks_header_navigation()` guard in `_rest_requested()` since it's now reachable from any screen instead of only the map). 江湖纪事 moved into the quest journal (`_show_quests()`), using the identical collapsible toggle+label pattern 0.97.0 established. `WorldMapView`'s `enter_requested`/`rest_requested` signals and its now-dead `_action_button()` helper were removed. `test_map_view.gd` extended to cover the empty side area, entering via marker click, and the new header rest button's wiring.
