...
青云工坊 (`scripts/progression/crafting_rules.gd`, `CraftingRules`) gained a fourth recipe, "炼制 · 悟性丹" (`insight_pill`, 3 herbs + 15 silver), permanently raising 悟性 by 1 the instant it's crafted — unlike 淬炼青锋's `MAX_FORGE_LEVEL` cap, there is deliberately **no ceiling** on how many can be crafted, mirroring how `train("insight")` (the existing attribute-training action) has no cap either. `CraftingRules.apply()`/`options()`/`inventory_text()` are the only places that needed a new branch — `GameState.craft()`, the workshop's `_rebuild()`/`_resolve_choice()` wiring in `main.gd`, and the Steam-milestone lookup all already dispatch generically by recipe id, so no UI code changes were needed to make the new recipe appear and function (the milestone lookup simply has no entry for `insight_pill`, so it grants no achievement flag — intentional, not an oversight). `test_crafting_rules.gd` covers its cost, its immediate uncapped effect, and that it stays independently repeatable rather than blocked by any level ceiling.
